...
pcapred.R maps external data into the PCA space generated by flashpca or plink; by default maps into the UK Biobank first 18 PCs.\
Author: Daniel Lawson: dan.lawson@bristol.ac.uk\
Version: 1.0.1\
Copyright University of Bristol 2020. All rights reserved.\
"
...
option_list = list(
    ...
    make_option(c("-r", "--ref"), type="character", default=NULL, 
                help="reference data package. If none provided, 18 PCs from UK Biobank are used. If provided the location should have the following set of files:
...
if(is.null(opt$out)){
    opt$out=gsub(".raw.gz","",opt$file,fixed=T)
    opt$out=gsub(".raw","",opt$out,fixed=T)
    opt$out=paste0(opt$out,".eigenvec.pred")
}
...
if(opt$verbose){
    ...
    if(is.null(opt$ref)) {
        cat(paste0("ref = ukb_pcs_18 from package \"pcapred\"\n"))
    }else{
    ...
    cat(paste0("output file = ",opt$out,"\n"))
    cat(paste0("minimum frequency = ",opt$minfreq,"\n"))
    cat(paste0("test = ",opt$test,"\n"))
...
if(is.null(opt$ref)){
    if(opt$verbose) cat("Using internal reference dataset ukb_pcs_18 from package pcapred")
    ref=ukb_pcs_18
}else{
...
if(opt$test){
    ...
    testres=test_pca(testfile,pred,opt$out,opt$verbose)    
}
